...
ligne = st.selectbox(“Ligne”, options_ligne, index=idx_l, key=f”{prefixe}*ligne”)
if ligne != “Toutes”:
machines = get_config(f”Machine*{ligne}”)
...
# --- F. GESTION DES STOCKS ---
    ...
    with tab_inv:
        ...
        if not df_s.empty:                
            ...
            # --- Suppression d'une pièce (admin uniquement) ---
            if is_admin:
                st.divider()
                st.subheader("🗑️ Supprimer une référence")
                options_del = df_s.apply(lambda r: f"{r['code_magasin']} — {r['designation']}", axis=1).tolist()
                sel_piece = st.selectbox("Sélectionner la pièce à supprimer", options_del, key="del_stock_sel")
                if st.button("❌ Supprimer définitivement cette référence", key="del_stock_btn"):
                    code_mag_del = sel_piece.split(" — ")[0].strip()
                    c.execute("DELETE FROM stocks WHERE code_magasin=?", (code_mag_del,))
                    conn.commit()
                    st.success(f"Référence '{sel_piece}' supprimée.")
                    st.rerun()
        else: 
    ...
    with tab_add:
        with st.form("form_stock"):
            ...
            c_ref = st.text_input("Reférence constructeur")
            c_des = st.text_input("Designation")
